...
|sd_cmd|双向|1|SD卡的cmd|
|sd_dat|输入|4|SD卡的数据线|
|fpioa|双向|可配置|现场可编程IO阵列|

其中，`config.v`的`CPU_CLOCK_HZ`必须修改为`clk`引脚的输入时钟频率。`sd_dat`的位宽为4，实际上仅sd_dat[0]有效。  
在BSP中，fpioa[0]默认分配为printf的串口打印端口，波特率115200。  

#### 程序固化
`$readmemh`是实现软件固化到处理器内部的关键。
与大部分Verilog教程所说的不同，对于现代FPGA平台，`initial` `$readmem` `reg xxx=1'b1`这些语法是可综合的，可以更便捷地实现寄存器、RAM赋初值。经过实测，高云、安路、紫光同创、AMD的FPGA都可以支持。  
在`/rtl/soc/iram.v`的最下面  
```
initial begin
    $readmemh (`PROG_FPGA_PATH, RAM);
end
```
使用`$readmemh`读入程序`inst.txt`，将其和RTL设计整合在一起写进FPGA。  
`PROG_FPGA_PATH`在`config.v`中指定为`inst.txt`的路径，在FPGA逻辑综合阶段读入。烧录比特流文件后，FPGA中对应的程序存储器RAM会初始化为`inst.txt`的数据，实现程序固化。  
...
小麻雀处理器存在2个时钟域  
1. 系统主时钟域`clk`  
2. JTAG模块时钟域`jtag_clk`(关闭JTAG则不存在)  
...
`CPU_CLOCK_HZ`**必须修改为处理器运行的频率**，具体数值和硬件板卡有关，软件通过读取此数值得到分频器系数。  
如果自建工程，需将`/rtl/config.v`的`PROG_FPGA_PATH`宏定义内容改为`inst.txt`的文件路径，通过`$readmemh`将程序写进FPGA。  
...
默认情况下，C语言的printf通过`fpioa[0]`以`115200`波特率的串口形式输出。通过`CH340`等USB转串口工具读取。  
同时引出了JTAG接口，支持`DAPLink`等调试器。  
`fpioa`可以接各种各样的外围电路。  

## 常见问题
**为什么LUT消耗得太多，FPGA资源不够报错？**  
正常情况下，小麻雀处理器需要消耗不大于10k的LUT资源，如果超得太多，可能是程序存储器综合失败了，主要原因如下：  
为了提高可移植性，程序存储器使用Verilog行为级建模，由综合器推断出等效的RAM硬核，而不是手动调用。但是，部分综合器的行为级推断很傻，内部的RAM硬核即使功能支持也用不上，强行将程序存储器综合成LUTRAM，消耗巨量的逻辑资源。  
...
|紫光同创|Pango Design Suite|2021.4-SP1.2|完美支持|
|安路|TD|5.6.1|config.v须配置IRAM_SPRAM_W4B|
|Lattice|Diamond|3.12|待测试|
|Intel|Quartus|18.1|我没有intel的FPGA|

**为什么综合报错，找不到define.v/config.v？**  
