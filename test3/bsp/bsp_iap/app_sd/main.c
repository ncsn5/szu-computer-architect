...
//#define IAP_DBG_MODE 1 //debug模式，需要占用更多的iram，必须修改link.lds的_iram_iap_size，给更大的空间
...
int main()
{
    ...
    while(i)
    {
        ...
        i--;
        for(j=0; j<512; j=j+4)//读出数据
        {
            tmp = read_mem_4byte(j);
            SYS_RWMEM_W(app_base_addr) = tmp;
            app_base_addr=app_base_addr+4;
        }
    }
#ifdef IAP_DBG_MODE
