...
复位整个处理器  
- `mdw 地址 [长度]`  
从指定地址读取32bit数据，可指定读取长度。比如`mdw 0 4`将以4字节为粒度读取地址0x00-0x10的数据  
- `load_image 文件名 起始地址 文件类型(bin,ihex,elf,s19) 最小地址 最大长度`  
从电脑加载镜像到处理器的指定地址  
- `verify_image 文件名 起始地址`  
校验镜像  
